...
/**
 * Reusable cascading dropdowns for regional info (State, District, Local Body, Gram Panchayat)
 *
 * Props:
 * - value: { State, District, local_body, gram_panchayat }
 * - onChange: (updatedValue) => void
 * - data: {
 *     STATES,
 *     STATE_TO_DISTRICTS,
 *     DISTRICT_TO_CITIES,
 *     DISTRICT_LOCAL_BODIES_MAP, // { [district]: { NAGAR_PALIKA: [], JANPAD_PANCHAYAT: [] } }
 *     DISTRICT_GRAM_PANCHAYATS_MAP // { [district]: { [localBody]: [] } }
 *   }
 * - disabled?: boolean
 */
const RegionalDropdowns = ({ value, onChange, data, disabled }) => {
  ...
  // Local body options: prefer per-district mapping if available, else fallback
