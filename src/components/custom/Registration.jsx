...
const RegistrationForm = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Create MPIN if it was stored during login
      const mpinToCreate = location.state?.mpinToCreate;
      if (mpinToCreate) {
        try {
          const mpinResponse = await fetch(`${API_BASE}/api/create-mpin`, {
            method: 'POST',
            headers: {
          ...
          if (!mpinResponse.ok) {
            console.error('Failed to create MPIN after registration');
          }
        } catch (error) {
