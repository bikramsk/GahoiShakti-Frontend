...
const Gallery = () => {
  ...
  // Add safety check for getting districts
  const getDistricts = (state) => {
    try {
      if (!state || !STATE_TO_DISTRICTS[state]) return [];
      return STATE_TO_DISTRICTS[state].filter(Boolean);
    } catch (error) {
  ...
  // Add safety check for getting local bodies
  const getLocalBodiesSafe = (district) => {
    try {
      if (!district) return [];
      const bodies = getLocalBodies(district);
      return Array.isArray(bodies) ? bodies.filter(Boolean) : [];
    } catch (error) {
      console.error('Error getting local bodies:', error);
      return [];
    }
  };

  // Add safety check for getting gram panchayats
  const getGramPanchayatsSafe = (district) => {
    try {
      if (!district) return [];
      const panchayats = getGramPanchayats(district);
      return Array.isArray(panchayats) ? panchayats.filter(Boolean) : [];
    } catch (error) {
      console.error('Error getting gram panchayats:', error);
      return [];
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-red-50 to-rose-100">
      ...
      {showRegistrationModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white w-full max-w-7xl mx-4 rounded-lg shadow-xl flex flex-col max-h-[90vh]">
            {/* Fixed Header */}
            <div className="px-6 py-4 bg-red-600 rounded-t-lg flex justify-between items-center">
              <div>
